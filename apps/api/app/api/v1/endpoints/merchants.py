...
async def update_my_merchant(
    ...
    updatable_fields = [
        "name", "logo", "category", "address", "phone",
        "description", "business_hours", "latitude", "longitude"
    ]
...
async def get_merchant_detail(
    ...
    total_redeemed = await db.execute(
        select(func.count()).select_from(RedeemOrder).where(
            and_(
...
async def redeem_item(
    ...
    # 获取商品
    result = await db.execute(
        select(RedeemItem).where(RedeemItem.id == request.item_id)
    )
    ...
    # 获取商家信息
    merchant_result = await db.execute(
        select(Merchant).where(Merchant.id == item.merchant_id)
    )
    merchant = merchant_result.scalar_one_or_none()
...
async def verify_order(
    ...
    # 获取订单
    result = await db.execute(
        select(RedeemOrder).where(RedeemOrder.id == order_id)
    )
    ...
    return RedeemOrderVerifyResponse(
        success=True,
        message="核销成功",
        order=RedeemOrderResponse.model_validate(order)
    )

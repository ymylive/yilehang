...
class Permission(Base):
    ...
    type: Mapped[str] = mapped_column(String(20), default=PermissionType.API.value, comment="权限类型")
    resource: Mapped[Optional[str]] = mapped_column(String(200), comment="资源路径（API路径/页面路径）")
    action: Mapped[Optional[str]] = mapped_column(String(50), comment="操作（GET/POST/PUT/DELETE等）")
...
class Menu(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    parent_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("menus.id"), comment="父菜单ID")

...
class MembershipCard(Base):
    ...
    # 关系
    student_memberships: Mapped[List["StudentMembership"]] = relationship(
        "StudentMembership", back_populates="card"
    )
...
class StudentMembership(Base):
    ...
    card: Mapped["MembershipCard"] = relationship(
        "MembershipCard", back_populates="student_memberships"
    )
    transactions: Mapped[List["Transaction"]] = relationship(
        "Transaction", back_populates="membership"
    )
...
class Booking(Base):
    ...
    membership_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("student_memberships.id")
    )  # 扣费的课时卡
    ...
    membership: Mapped[Optional["StudentMembership"]] = relationship(
        "StudentMembership", back_populates="bookings"
    )
    review: Mapped[Optional["Review"]] = relationship(
        "Review", back_populates="booking", uselist=False
    )
    coach_feedback: Mapped[Optional["CoachFeedback"]] = relationship(
        "CoachFeedback", back_populates="booking", uselist=False
    )
    transactions: Mapped[List["Transaction"]] = relationship(
        "Transaction", back_populates="booking"
    )
...
class Transaction(Base):
    ...
    # 关系
    membership: Mapped[Optional["StudentMembership"]] = relationship(
        "StudentMembership", back_populates="transactions"
    )

...
class EnergyAccount(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    student_id: Mapped[int] = mapped_column(Integer, ForeignKey("students.id"), unique=True, index=True)
    ...
    transactions: Mapped[List["EnergyTransaction"]] = relationship("EnergyTransaction", back_populates="account")
...
class EnergyTransaction(Base):
    ...
    __table_args__ = (
        Index("ix_energy_transactions_student_created", "student_id", "created_at"),
    )
    ...
    rule_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("energy_rules.id"))  # 关联规则

...
import { coachStudentsApi, coachScheduleApi, coachFeedbackApi } from '@/api/index'
...
function goToFeedback() {
  ...
  uni.navigateTo({
    url: `/pages/coach/students/feedback?studentId=${student.value.id}&studentName=${encodeURIComponent(student.value.name)}`
  })
}
...
async function loadStudent() {
  const data = await coachStudentsApi.getStudent(studentId.value)
...
async function loadRecentLessons() {
  ...
  const data: any = await coachScheduleApi.getSchedule({
...
async function loadRecentFeedbacks() {
  const data: any = await coachFeedbackApi.getFeedbacks({
    page: 1,
    page_size: 20,

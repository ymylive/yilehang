...
async function loadBookingDetail() {
  ...
  loading.value = true
  try {
    const data = await coachScheduleApi.getBookingDetail(bookingId.value)
...
function completeBooking() {
  ...
  uni.showModal({
    ...
    success: async (res) => {
      ...
      try {
        await coachScheduleApi.completeBooking(booking.value.id)
...
function goToFeedback() {
  if (!booking.value) return
  uni.navigateTo({
    url: `/pages/coach/students/feedback?studentId=${booking.value.student_id}&bookingId=${booking.value.id}&studentName=${encodeURIComponent(booking.value.student_name)}`
  })
}
